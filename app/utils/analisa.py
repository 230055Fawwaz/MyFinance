...
def hitung_kesehatan_keuangan(df):
    ...
    total_pemasukan = (
        df_bulan_ini[df_bulan_ini["cat_type"] == "income"]["amount"].astype(float).sum()
    )
    total_pengeluaran = (
        df_bulan_ini[df_bulan_ini["cat_type"] == "expense"]["amount"]
        .astype(float)
        .abs()
        .sum()
    )
...
def deteksi_anomali_pengeluaran(df):
    ...
    return anomali_df[["id", "date", "kategori", "amount", "note"]].to_dict(
        orient="records"
    )
...
def analisis_hubungan_kategori(df):
    ...
    basket = (
        df_expense.groupby(["tanggal", "sub_nama"])["sub_nama"]
        .count()
        .unstack()
        .reset_index()
        .fillna(0)
