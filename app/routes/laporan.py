...
def ambil_data_agregasi(start_date_str, end_date_str):
    ...
    subcategories = query_sub.group_by(Category.id, Category.nama, SubCategory.id, SubCategory.nama).all()
...
def download_csv():
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=laporan_pengeluaran_{start_date}_to_{end_date}.csv'
    return response
...
def download_pdf():
    ...
    if start_date and end_date:
        story.append(Paragraph(f"<font color='grey'>Periode: {start_date} s/d {end_date}</font>", styles['Normal']))
    ...
    for cat in categories:
        ...
        table_styles.append(('BACKGROUND', (0, row_idx), (-1, row_idx), colors.HexColor('#f8f9fa')))
        row_idx += 1
        
        for sub in subcategories:
            if sub.category_name == cat.category_name:
                table_data.append([f"  • {sub.subcategory_name}", f"Rp {sub.total:,.2f}"])
                table_styles.append(('TEXTCOLOR', (0, row_idx), (-1, row_idx), colors.HexColor('#6c757d')))
                row_idx += 1
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=laporan_pengeluaran_{start_date}_to_{end_date}.pdf'
    return response
